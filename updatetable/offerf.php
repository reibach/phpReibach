...
if (!$query)
	...
	while (!$query->EOF)
	{
		...
		$query1 = $db->GetAll("SELECT MESSAGEID, DESCRIPTION FROM message WHERE MESSAGEID=$messageID");
		...
		$query3 = $db->GetAll("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay WHERE METHODOFPAYID=$MethodOfPayment");
		...
		$query4 = $db->Execute("UPDATE updatetable SET TABLEUPDATE='1', CREATED='$CurrentDateTime' WHERE UPDATEID=1");
