...
if(empty($Order) || $Order !== 'METHODOFPAYID' && $Order !== 'DESCRIPTION')
{
	$Order = "DESCRIPTION";
	$Sort = "";
}

$smarty->assign("Title","$a[method_of_payment] - $a[searchresult]");
...
// Get MethodOfPayment Information
//
$query = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort");
	...
	// Save all entrys in $MethodOfPaymentData
	//
	foreach($query as $result)
	{
		$MethodOfPaymentData[] = $result;
	}

	$smarty->assign("PageRows","$pagenumrows");
	$smarty->assign("MaxRows","$numrows");
	$smarty->assign('MethodOfPaymentData', $MethodOfPaymentData);
...
$smarty->display('methodofpayment/searchlist.tpl');
