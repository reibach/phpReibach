...
$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, P.PAYMENTID, P.MYID, P.INVOICEID, DATE_FORMAT(P.PAYMENT_DATE,'%d.%m.%Y') AS PAYMENT_DATE, P.SUM_PAID, P.METHODOFPAYID,
		P.CARDNR, P.VALIDTHRU, P.SUM_PAID, P.NOTE, P.CANCELED, P.CREATEDBY, P.USERGROUP1, P.USERGROUP2, M.METHODOFPAYID, M.DESCRIPTION FROM payment AS P, addressbook AS A,
		methodofpay AS M WHERE A.MYID=P.MYID AND M.METHODOFPAYID=P.METHODOFPAYID AND P.PAYMENTID=$paymentID");
