...
{
	$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, P.CREATEDBY, P.PAYMENTID, P.MYID, DATE_FORMAT(P.PAYMENT_DATE,'%d.%m.%Y') AS PAYMENT_DATE, P.SUM_PAID, P.METHOD_OF_PAY, P.CANCELED FROM payment AS P, addressbook AS A WHERE P.CANCELED=1 AND A.MYID=P.MYID ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
}
else if(isset($Canceled) && $Canceled == 3)
{
	$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, P.CREATEDBY, P.PAYMENTID, P.MYID, DATE_FORMAT(P.PAYMENT_DATE,'%d.%m.%Y') AS PAYMENT_DATE, P.SUM_PAID, P.METHOD_OF_PAY, P.CANCELED FROM payment AS P, addressbook AS A WHERE A.MYID=P.MYID ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
}
else
{
...
if (!$query)
	...
	if(isset($Canceled) && $Canceled == 1)
	{
		$query1 = $db->Execute("SELECT SUM_PAID FROM payment WHERE CANCELED=1");
	}
	else if(isset($Canceled) && $Canceled == 3)
	{
		$query1 = $db->Execute("SELECT SUM_PAID FROM payment");
	}
	else
	{
		$query1 = $db->Execute("SELECT SUM_PAID FROM payment WHERE CANCELED=2");
	}
