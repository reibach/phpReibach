		...
		<td class="phprechnung_tabelle">
			<table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 2">
				...
				<tr>
					...
					[&nbsp;<a title="{$BackMsg} - {$Offer} - {$Edit}" class="ninfolink" href="edit.php?page={$page}&amp;myID={$myID}&amp;offerID={$offerID}&amp;tmpID=1&amp;messageID={$messageID}&amp;OfferDate={$OfferDate}&amp;OfferStatus={$OfferStatus}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;OfferID1={$OfferID1}&amp;CustomerID1={$CustomerID1}&amp;DateFrom1={$DateFrom1}&amp;DateTill1={$DateTill1}&amp;Total1={$Total1}&amp;Customer1={$Customer1}&amp;Order={$Order}&amp;Sort={$Sort}&amp;Canceled={$Canceled}&amp;{$Session}#{$offerID}">{$BackMsg}</a>&nbsp;]
					</td>
				</tr>
				{else}
				<tr>
					<td valign="middle" align="left" colspan="2">
					[&nbsp;<a title="{$BackMsg} - {$Offer} - {$New}" class="ninfolink" href="new.php?page={$page}&amp;myID={$myID}&amp;offerID={$offerID}&amp;tmpID=1&amp;messageID={$messageID}&amp;OfferDate={$OfferDate}&amp;OfferStatus={$OfferStatus}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;Order={$Order}&amp;Sort={$Sort}&amp;Canceled={$Canceled}&amp;{$Session}#{$offerID}">{$BackMsg}</a>&nbsp;]
					</td>
				</tr>
				{/if}
				<tr>
				...
				<tr>
					<td align="right" valign="top" width="30%">{$PositionName}:</td>
					...
					<input type="hidden" name="TMP_OfferID" value="2" />
					{if $infoID eq 9}
					...
					<input class="form_input" type="text" name="PosID" size="30" title="{$PositionSearch}" value="{$POS_NAME}" />&nbsp;<input class="button" type="submit" title="{$PositionChoose}" value="{$Search}" />
					</form>
					</td>
				</tr>
			</table>
				...
				<tr>
					...
					<table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 4">
						...
						<tr>
							...
							<input class="button" type="submit" name="OK" title="{$InsertMsg} - {$Offer} - {$New}" value="{$InsertMsg}" />
							</td>
						</tr>
					</table>
					</form>
