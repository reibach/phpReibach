...
{else}
	<body onload="document.New.Customer.focus();">
{/if}
...
<tr>
	...
	{* Menubar start *}
		<table border="0" width="80%" cellspacing="0" cellpadding="0" summary="Tabelle 4">
			...
			<tr>
				<td align="left" class="phprechnung_menu"><a accesskey="O" title="{$Offer} - {$List}"
				href="list.php?{$Session}">{$Offer}</a>
				</td>
			</tr>
			<tr>
				<td align="left" class="phprechnung_menu_sel"><a accesskey="1" title="{$Offer} - {$New}"
			...
			<tr>
				<td class="phprechnung_tabelle">
					<table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 2">
						<tr>
						{if $TaxFree != 1}
							<td align="center" colspan="8"><h2>{$Offer} - {$New}</h2>
							</td>
						{else}
							<td align="center" colspan="7"><h2>{$Offer} - {$New}</h2>
							</td>
						{/if}
						</tr>
						{if $smarty.session.NewID and ( $smarty.session.NewID eq 1 )}
							{if $TaxFree != 1}
								<tr>
									<td align="center" colspan="8" class="greentxt">{$NewEntry} {$Offer_No} {$offerID}
									</td>
								</tr>
							{else}
								<tr>
									<td align="center" colspan="7" class="greentxt">{$NewEntry} {$Offer_No} {$offerID}
									</td>
								</tr>
							{/if}
						{/if}
						...
						<tr>
							...
							<form id="New" name="New" action="customerlist.php?{$Session}" method="post">
								...
								<input type="hidden" name="messageID" value="{$NR_MESSAGE}" />
						...
						{if $MYID}
							...
							<tr>
								...
								<form id="MethodOfPayD" name="MethodOfPayD" action="{$smarty.server.PHP_SELF}?{$Session}" method="post">
									...
									<input type="hidden" name="Order" value="{$Order}" />
									<input type="hidden" name="Sort" value="{$Sort}" />
									<input type="text" name="MethodOfPaymentDate" size="20" title="{$Payment }{$Date_Till} {$DateMsg}" value="{$MethodOfPaymentDate}" onchange="this.form.submit();" />
								</form>
								</td>
							</tr>
							<tr>
					...
					<table width="100%" border="0" cellspacing="1" cellpadding="3" summary="Tabelle 3">
						<tr class="mblueTD">
							<td nowrap="nowrap" align="left" valign="middle">{$PositionName}&nbsp;-&nbsp;[&nbsp;<a title="{$PositionNew}" class="nlink" href="posnew.php?myID={$myID}&amp;messageID={$NR_MESSAGE}&amp;OfferDate={$OFFER_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}">&nbsp;{$New}&nbsp;</a>&nbsp;]
							</td>
						...
						{foreach from=$PosData item=offer}
							{if $smarty.session.Username and ( $smarty.session.Username eq $offer.USERNAME )}
								<tr class="{cycle values="grayTD,wTD"}">
									...
									<td valign="top" align="center"><a href="posedit.php?tmpPosID={$offer.TMP_OFFERID}&amp;myID={$MYID}&amp;posID={$offer.POSITIONID}&amp;messageID={$NR_MESSAGE}&amp;OfferDate={$OFFER_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"><img border="0" src="../images/edit.png" title="{$Position} - {$Edit}" alt="{$Position} - {$Edit}" /></a>
									</td>
									<td valign="top" align="center"><a href="posdelete.php?tmpPosID={$offer.TMP_OFFERID}&amp;myID={$MYID}&amp;messageID={$NR_MESSAGE}&amp;OfferDate={$OFFER_DATE}&amp;MethodOfPayment={$NR_METHOD_OF_PAYMENT}&amp;MethodOfPaymentDate={$MethodOfPaymentDate}&amp;Note={$Note}&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"><img border="0" src="../images/delete.png" title="{$Position} - {$Delete}" alt="{$Position} - {$Delete}" /></a>
									</td>
							</tr>
							{/if}
						{/foreach}
						...
						<tr>
							...
							<input type="hidden" name="MethodOfPaymentDate" value="{$MethodOfPaymentDate}" />
							<input type="hidden" name="Note" value="{$Note}" />
							<input type="hidden" name="mark" value="New1.SaveOffer" />
							<input type="hidden" name="page" value="{$page}" />
							<input type="hidden" name="Order" value="{$Order}" />
							<input type="hidden" name="Sort" value="{$Sort}" />
							...
							{foreach from=$MessageData item=message}
								{if $NR_MESSAGE == $message.MESSAGEID}
							...
							<form id="Print" name="Print" action="print_pdf.php?{$Session}" target="_blank" method="post">
								...
								{else}
									<input class="button" type="submit" title="{$Print_Offer}" value="{$Print}" disabled="disabled" />
								{/if}
							</form>
							</td>
							{if $TaxFree neq 1}
							...
							<form id="New1" name="New1" action="newf.php?{$Session}" method="post">
								...
								<input type="hidden" name="Note" value="{$Note}" />
								<input type="hidden" name="messageID" value="{$NR_MESSAGE}" />
								<input class="button" name="SaveOffer" type="submit" title="{$Save_Offer}" value="{$Save}" />
							</form>
							</td>
						</tr>
					</table>
