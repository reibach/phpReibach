...
<form id="ChangeStatus" name="ChangeStatus" action="change_statusf.php?{$Session}" method="post">
...
{/if}
<tr>
	<td align="center" colspan="2"><h2>{$Offer} - {$Change_Status}</h2><br />
	{$FIRSTNAME}&nbsp;{$LASTNAME}&nbsp;{$COMPANY}<br />
	{$Offer_No}: {$OfferInitials}-{$PrintDate}, {$Customer_No}: {$myID}<br /></td>
</tr>
<tr><td></td></tr>
<tr>
<td valign="middle" align="right" width="40%">{$Offer_Status}:</td>
<td valign="middle" align="left" width="60%">
<select class="choice200" name="OfferStatus" title="{$Offer_Status}">
<optgroup label="{$Offer_Status}" title="{$Offer_Status}">
{foreach item=offerstatus from=$change_status}
	{foreach key=key item=item from=$offerstatus}
		{if $STATUS and ( $STATUS == $key)}
...
<input type="submit" class="button" title="{$Offer} - {$Change_Status}" value="{$Change_Status}" />
</td>
</tr>
</table></td></tr></table>
</form>
