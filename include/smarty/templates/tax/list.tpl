...
<tr><td align="center" colspan="5"><h2>{$Tax} - {$List}</h2></td></tr>
<tr><td>&nbsp;</td></tr>
...
{if $smarty.session.EditID and ( $smarty.session.EditID eq 1 )}
	<tr><td align="center" colspan="4" class="greentxt">{$EntryChanged} {$EntryNo} {$taxID}</td></tr>
{/if}
{if $smarty.session.DeleteID and ( $smarty.session.DeleteID eq 1 )}
	<tr><td align="center" colspan="4" class="greentxt">{$EntryDeleted} {$EntryNo} {$taxID}</td></tr>
{/if}
<tr><td></td></tr>
<tr class="mblueTD">
<td nowrap="nowrap" align="left">{$EntryNo}
<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=TAXID&amp;Sort=ASC&amp;{$Session}"><img border="0" src="../images/up.png" title="{$SortMsg} {$EntryNo} ASC" alt="{$SortMsg} {$EntryNo} ASC" /></a>&nbsp;<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=TAXID&amp;Sort=DESC&amp;{$Session}"><img border="0" src="../images/down.png" title="{$SortMsg} {$EntryNo} DESC" alt="{$SortMsg} {$EntryNo} DESC" /></a>&nbsp;</td>
<td nowrap="nowrap" align="left">{$Tax_Description}
<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=TAX_DESC&amp;Sort=ASC&amp;{$Session}"><img border="0" src="../images/up.png" title="{$SortMsg} {$Tax_Description} ASC" alt="{$SortMsg} {$Tax_Description} ASC" /></a>&nbsp;<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=TAX_DESC&amp;Sort=DESC&amp;{$Session}"><img border="0" src="../images/down.png" title="{$SortMsg} {$Tax_Description} DESC" alt="{$SortMsg} {$Tax_Description} DESC" /></a>&nbsp;</td>
...
{else}
{foreach from=$TaxData item=tax}
	<tr class="{cycle values="grayTD,wTD"}">
	<td valign="top" align="left"><a name="{$tax.TAXID}" title="{$AllInformation} {$EntryNo} {$tax.TAXID} - {$tax.TAX_DESC}" class="ninfolink" href="info.php?taxID={$tax.TAXID}&amp;{$AddCurrentPage}Order={$Order}&amp;Sort={$Sort}&amp;{$Session}">
	{if $tax.TAXID}
		{$tax.TAXID}
	{else}
		&nbsp;i&nbsp;
	{/if}</a></td><td valign="top" align="left">{$tax.TAX_DESC}</td>
	{if $smarty.session.Username and ( $smarty.session.Username == $Root or $smarty.session.Usergroup1 == $AdminGroup1 or $smarty.session.Usergroup2 == $AdminGroup2)}
		<td valign="top" align="center"><a href="edit.php?taxID={$tax.TAXID}&amp;{$AddCurrentPage}Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"><img border="0" src="../images/edit.png" title="{$Editentry}" alt="{$Editentry}" /></a></td></tr>
	{else}
		<td></td></tr>
	{/if}
{/foreach}
