...
<tr><td align="center" colspan="5"><h2>{$MethodOfPayment} - {$List}</h2></td></tr>
<tr><td>&nbsp;</td></tr>
...
<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=METHODOFPAYID&amp;Sort=ASC&amp;{$Session}"><img border="0" src="../images/up.png" title="{$SortMsg} {$EntryNo} ASC" alt="{$SortMsg} {$EntryNo} ASC" /></a>&nbsp;<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=METHODOFPAYID&amp;Sort=DESC&amp;{$Session}"><img border="0" src="../images/down.png" title="{$SortMsg} {$EntryNo} DESC" alt="{$SortMsg} {$EntryNo} DESC" /></a>&nbsp;</td>
<td nowrap="nowrap" align="left">{$MethodOfPayment}
<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=DESCRIPTION&amp;Sort=ASC&amp;{$Session}"><img border="0" src="../images/up.png" title="{$SortMsg} {$Method_Of_Payment} ASC" alt="{$SortMsg} {$Method_Of_Payment} ASC" /></a>&nbsp;<a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=DESCRIPTION&amp;Sort=DESC&amp;{$Session}"><img border="0" src="../images/down.png" title="{$SortMsg} {$Method_Of_Payment} DESC" alt="{$SortMsg} {$Method_Of_Payment} DESC" /></a>&nbsp;</td>
...
{foreach from=$MethodOfPaymentData item=methodpay}
	...
	<td valign="top" align="left"><a name="{$methodpay.METHODOFPAYID}" title="{$AllInformation} {$EntryNo} {$methodpay.METHODOFPAYID} - {$methodpay.DESCRIPTION}" class="ninfolink" href="info.php?methodpayID={$methodpay.METHODOFPAYID}&amp;{$AddCurrentPage}Order={$Order}&amp;Sort={$Sort}&amp;{$Session}">
	{if $methodpay.METHODOFPAYID}
		{$methodpay.METHODOFPAYID}
	{else}
		&nbsp;i&nbsp;
	...
	{if $smarty.session.Username and ( $smarty.session.Username == $Root or $smarty.session.Usergroup1 == $AdminGroup1 or $smarty.session.Usergroup2 == $AdminGroup2 and $methodpay.METHODOFPAYID != 2)}
		<td valign="top" align="center"><a href="edit.php?methodpayID={$methodpay.METHODOFPAYID}&amp;{$AddCurrentPage}Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"><img border="0" src="../images/edit.png" title="{$Editentry}" alt="{$Editentry}" /></a></td></tr>
	{else}
		<td></td></tr>
	{/if}
{/foreach}
