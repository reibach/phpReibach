...
<tr><td align="center" colspan="2"><h2>{$MethodOfPayment} - {$Info}</h2></td></tr>
<tr><td></td></tr>
{if $smarty.session.Username and ( $smarty.session.Username eq $Root and $methodpayID != 2)}
	<tr><td align="center" colspan="2">
	[&nbsp;<a title="{$Editentry}" class="nmenulink" href="edit.php?methodpayID={$methodpayID}&amp;infoID={$infoID}&amp;page={$page}&amp;Order={$Order}&amp;Sort={$Sort}&amp;Description_1={$Description_1}&amp;{$Session}">{$Edit}</a>
	&nbsp;|&nbsp;<a title="{$Deleteentry}" class="nmenulink" href="delete.php?methodpayID={$methodpayID}&amp;infoID={$infoID}&amp;page={$page}&amp;Order={$Order}&amp;Sort={$Sort}&amp;Description_1={$Description_1}&amp;{$Session}">{$Delete}</a>&nbsp;]
	</td></tr>
{/if}
<tr><td></td></tr>
<tr><td align="center" colspan="2">{$AllInformation} {$EntryNo} {$methodpayID}</td></tr>
<tr><td></td></tr>
<tr><td valign="middle" align="right" width="40%">{$MethodOfPayment}:&nbsp;</td><td class="dbTxt" valign="middle" align="left" width="60%">{$METHODOFPAYMENT}</td></tr>
<tr><td></td></tr>
{* Display back button *}
<tr><td valign="middle" align="center" width="100%" colspan="2">
{if $infoID == 9}
	<form action="searchlist.php?{$Session}#{$methodpayID}" method="post">
...
{else}
	<form action="list.php?{$Session}#{$methodpayID}" method="post">
	<input type="hidden" name="page" value="{$page}" />
	<input type="hidden" name="Order" value="{$Order}" />
	<input type="hidden" name="Sort" value="{$Sort}" />
