...
$query = $db->Execute("SELECT SETTINGID, DATE_FORMAT(COMPANY_DATE,'%d.%m.%Y') AS COMPANY_DATE, PRINT_COMPANY_DATA, PRINT_POSITION_NAME, TAX_FREE, COMPANY_NAME, COMPANY_ADDRESS, COMPANY_POSTAL,
		COMPANY_CITY, COMPANY_COUNTRY, COMPANY_PHONE, COMPANY_FAX, COMPANY_EMAIL, COMPANY_URL, COMPANY_MOBILE, COMPANY_CURRENCY, COMPANY_SALESPRICE, COMPANY_TAXNR,
		...
		PDF_COMPANY_LOGO_HEIGHT, PDF_COMPANY_LOGO_WIDTH, PDF_FONT, PDF_DIR, PDF_FONT_SIZE1, PDF_FONT_SIZE2, PDF_TYPE_HEIGHT, PDF_ATTACHMENT_TEXT FROM setting");
...
if (!$query)
	...
	foreach($query as $f)
	{
		...
		$CompanyMobile = $f['COMPANY_MOBILE'];
