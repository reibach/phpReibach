...
$smarty->assign("Title","$a[position] - $a[pos_group] - $a[list]");
...
//
$query = $db->Execute("SELECT POSGROUPID, DESCRIPTION FROM posgroup ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT POSGROUPID FROM posgroup");
...
$smarty->display('posgroup/list.tpl');
