...
if(!isset($tmpID))
{
	$db->Execute("DELETE FROM tmp_invoice WHERE USERNAME='$_SESSION[Username]'");
}
...
if(!empty($offerID) && is_numeric($offerID))
{
	...
	$query = $db->Execute("SELECT OFFERPOSID, MYID, OFFERID, POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_DESC, TAX_MULTI, TAX_DIVIDE FROM offerpos WHERE OFFERID=$offerID ORDER BY POS_GROUP ASC, OFFERPOSID ASC");
	...
	if (!$query)
		...
		foreach($query as $f)
		{
			...
			$query1 = "INSERT INTO tmp_invoice (TMP_INVOICEID, MYID, INVOICEID, POSITIONID, USERNAME, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_MULTI, TAX_DIVIDE, TAX_DESC)";
...
if(!empty($myID) && is_numeric($myID))
{
	$query = $db->Execute("SELECT PREFIX, TITLE, FIRSTNAME, LASTNAME, ADDRESS, COMPANY, POSTALCODE, PRINT_NAME,
		CITY, COUNTRY, METHODOFPAY, MESSAGE, MYID FROM addressbook WHERE MYID=$myID");
...
//
$query2 = $db->GetRow("SELECT MAX(INVOICEID) AS MAX_INVOICEID FROM invoice");
if (!$query2)
	print($db->ErrorMsg());
else
...
$posquery = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, T.USERNAME, T.POSITIONID, T.POS_DESC, T.POS_QUANTITY, T.POS_PRICE, T.TAX, T.TAX_DIVIDE, T.TAX_MULTI, T.TAX_DESC, T.POS_GROUP, T.TMP_INVOICEID FROM article AS P, tmp_invoice AS T WHERE P.POSITIONID=T.POSITIONID ORDER BY T.POS_GROUP ASC, T.TMP_INVOICEID ASC");
...
//
$query5 = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay ORDER BY DESCRIPTION ASC");
$query6 = $db->Execute("SELECT MESSAGEID, DESCRIPTION FROM message ORDER BY DESCRIPTION ASC");
