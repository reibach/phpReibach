...
$query = $db->Execute("SELECT A.PREFIX, A.TITLE, A.FIRSTNAME, A.LASTNAME, A.ADDRESS, A.COMPANY, A.POSTALCODE, A.PRINT_NAME,
	...
	FROM addressbook AS A, invoice AS I WHERE A.MYID=I.MYID AND I.INVOICEID=$invoiceID");
...
if (!$query)
	...
	V.POS_QUANTITY, V.POS_PRICE, V.TAX, V.TAX_DIVIDE, V.TAX_MULTI, V.TAX_DESC FROM article AS P, invoicepos AS V WHERE P.POSITIONID=V.POSITIONID AND V.INVOICEID=$invoiceID ORDER BY V.POS_GROUP ASC, V.INVOICEPOSID ASC");
	...
	$paid = $db->Execute("SELECT METHOD_OF_PAY, INVOICEID, SUM_PAID, DATE_FORMAT(PAYMENT_DATE,'%d.%m.%Y') AS PAYMENT_DATE
			FROM payment WHERE CANCELED=2 AND INVOICEID=$invoiceID");
	...
	else
	{
		...
		if(!empty($EmailTo))
		{
			...
			if($PHPSendMail == "1")
			{
				PEAR_Email_Attachment($EmailTo, $EmailCc, $EmailBcc, $EmailPriority, $Subject, $CompanyPdfAttachmentText, $sendfile, $_SESSION['Charset']);
			}
			else
			{
				Email_Attachment($EmailTo, $EmailCc, $EmailBcc, $EmailPriority, $Subject, $CompanyPdfAttachmentText, $sendfile, $_SESSION['Charset']);
			}
