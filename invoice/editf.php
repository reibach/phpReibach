...
else
{
	...
	$query1 = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay WHERE METHODOFPAYID=$MethodOfPayment");
	...
	$db->Execute("DELETE FROM invoicepos WHERE INVOICEID=$invoiceID");

	$query2 = $db->Execute("SELECT TMP_INVOICEID, POSITIONID, USERNAME, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_MULTI, TAX_DIVIDE, TAX_DESC FROM tmp_invoice WHERE USERNAME='$_SESSION[Username]' ORDER BY TMP_INVOICEID");
	...
	if (!$query2)
		...
		foreach($query2 as $f)
		{
			...
			$query3 = "INSERT INTO invoicepos (INVOICEPOSID, MYID, INVOICEID, POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_DESC, TAX_MULTI, TAX_DIVIDE)";
