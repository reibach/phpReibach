...
//
$query = $db->Execute("SELECT INVOICEID, CREATEDBY FROM invoice WHERE INVOICEID=$invoiceID");
...
else
{
	...
	$query1 = $db->Execute("SELECT MYID, MESSAGEID, MESSAGE_DESC, METHODOFPAYID, METHOD_OF_PAY,
			TAX1_TOTAL, TAX2_TOTAL, TAX3_TOTAL, TAX4_TOTAL, TAX1_DESC, TAX2_DESC, TAX3_DESC, TAX4_DESC, SUBTOTAL1, SUBTOTAL2, SUBTOTAL3, SUBTOTAL4, TOTAL_AMOUNT, NOTE FROM invoice WHERE INVOICEID=$invoiceID");
	...
	//
	$query3 = $db->GetRow("SELECT MAX(INVOICEID) AS MAX_INVOICEID FROM invoice");
	if (!$query3)
		die($db->ErrorMsg());
	else
	...
	//
	$query4 = $db->Execute("SELECT POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_DESC, TAX_MULTI, TAX_DIVIDE FROM invoicepos WHERE INVOICEID=$invoiceID");
	...
	if (!$query4)
		...
		foreach($query4 as $f)
		{
			...
			//
			$query5 = "INSERT INTO invoicepos (INVOICEPOSID, MYID, INVOICEID, POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_DESC, TAX_MULTI, TAX_DIVIDE)";
