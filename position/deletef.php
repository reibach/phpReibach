...
else
{
	...
	//
	$query1 = $db->Execute("SELECT POSITIONID FROM invoicepos WHERE POSITIONID=$posID");
	$query2 = $db->Execute("SELECT POSITIONID FROM offerpos WHERE POSITIONID=$posID");
	...
	{
		$query4 = "DELETE FROM article WHERE POSITIONID=$posID";
