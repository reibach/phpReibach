...
if(empty($Order) || $Order !== 'POSGROUPID' && $Order !== 'DESCRIPTION')
{
	$Order = "DESCRIPTION";
	$Sort = "";
}

$smarty->assign("Title","$a[position] - $a[pos_group] - $a[searchresult]");
...
//
$query = $db->Execute("SELECT POSGROUPID, DESCRIPTION FROM posgroup WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT POSGROUPID, DESCRIPTION FROM posgroup WHERE DESCRIPTION LIKE '%$Description_1%' ORDER BY $Order $Sort");
	...
	// Save all entrys in $PosGroupData
	//
	...
	if(isset($PosGroupData))
		$smarty->assign('PosGroupData', $PosGroupData);
...
$smarty->display('posgroup/searchlist.tpl');
