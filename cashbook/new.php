...
$query = $db->GetRow("SELECT MAX(CASHBOOKID) AS MAX_CASHBOOKID FROM cashbook");
if (!$query)
	print($db->ErrorMsg());
else
...
//
$query1 = $db->Execute("SELECT SUM(CASH_IN_HAND_STARTING_WITH) AS CASH_IN_HAND_STARTING_WITH FROM cashbook WHERE CANCELED=2");
