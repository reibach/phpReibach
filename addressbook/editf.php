...
// Get the creator
//
$query = $db->Execute("SELECT MYID, CREATEDBY FROM addressbook WHERE MYID=$myID");
$row = $query->GetRows();
...
{
	$query1 = $db->Execute("SELECT MYID, FIRSTNAME, LASTNAME, ADDRESS, POSTALCODE, CITY FROM addressbook WHERE FIRSTNAME='$Firstname' AND LASTNAME='$Lastname' AND ADDRESS='$Address' AND POSTALCODE='$Postalcode' AND CITY='$City' AND MYID != $myID");
