...
//
$query1 = $db->Execute("SELECT CATEGORYID, DESCRIPTION FROM category ORDER BY DESCRIPTION ASC");
$query2 = $db->Execute("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay ORDER BY DESCRIPTION ASC");
...
if ((!$query1) || (!$query2))
	...
	$query3 = $db->Execute("SELECT MESSAGEID, DESCRIPTION FROM message ORDER BY DESCRIPTION ASC");
	...
	//
	$query4 = $db->GetRow("SELECT MAX(MYID) AS MAX_MYID FROM addressbook");
	if (!$query4)
		print $db->ErrorMsg();
	else
