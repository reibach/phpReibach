...
//
$query = $db->Execute("SELECT OFFERID, INVOICEID, CREATEDBY FROM offer WHERE OFFERID=$offerID");
...
else
{
	...
	$query1 = $db->Execute("SELECT MYID, MESSAGEID, MESSAGE_DESC, METHODOFPAYID, METHOD_OF_PAY, METHOD_OF_PAY_DATE,
			TAX1_TOTAL, TAX2_TOTAL, TAX3_TOTAL, TAX4_TOTAL, TAX1_DESC, TAX2_DESC, TAX3_DESC, TAX4_DESC, SUBTOTAL1, SUBTOTAL2, SUBTOTAL3, SUBTOTAL4, TOTAL_AMOUNT, NOTE FROM offer WHERE OFFERID=$offerID");
	...
	//
	$query3 = $db->GetRow("SELECT MAX(OFFERID) AS MAX_OFFERID FROM offer");
	if (!$query3)
		die($db->ErrorMsg());
	else
	...
	//
	$query4 = $db->GetAll("SELECT POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_MULTI, TAX_DIVIDE, TAX_DESC FROM offerpos WHERE OFFERID=$offerID");
	...
	if (!$query4)
		...
		foreach($query4 as $f)
		{
			...
			//
			$query5 = "INSERT INTO offerpos (OFFERPOSID, OFFERID, MYID, POSITIONID, POS_DESC, POS_QUANTITY, POS_PRICE, POS_GROUP, TAX, TAX_DESC, TAX_MULTI, TAX_DIVIDE)";
