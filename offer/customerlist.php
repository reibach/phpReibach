...
//
$query = $db->Execute("SELECT LASTNAME, FIRSTNAME, COMPANY, MYID FROM addressbook
...
if (!$query)
	...
	// Count search result
	//
	$query1 = $db->Execute("SELECT LASTNAME, FIRSTNAME, COMPANY, MYID FROM addressbook
		WHERE FIRSTNAME LIKE '%$Customer%' OR LASTNAME LIKE '%$Customer%' OR COMPANY LIKE '%$Customer%'");
...
$smarty->display('offer/customerlist.tpl');
