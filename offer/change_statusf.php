...
//
$query = $db->Execute("SELECT OFFERID, INVOICEID, CREATEDBY, MESSAGEID, METHODOFPAYID, NOTE FROM offer WHERE CANCELED=2 AND OFFERID=$offerID");
...
else
{
	...
	//
	$query1 = $db->Execute("SELECT INVOICEID FROM invoice WHERE CANCELED=2 AND INVOICEID=$invoiceID");
	...
	if (!$numrows1)
	{
		...
		{
			$query3 = "UPDATE offer SET STATUS='$OfferStatus', MODIFIEDBY='$_SESSION[Username]', MODIFIED='$CurrentDateTime' WHERE OFFERID=$offerID";
