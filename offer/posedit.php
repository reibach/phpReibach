...
if(isset($PosID) && is_numeric($PosID) && !empty($PosID))
{
	$PosID = $PosID;
	$smarty->assign("PosID",$PosID);
	$query1 = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, P.POS_DESC, P.POS_ACTIVE, P.POS_PRICE, P.POS_TAX, P.POS_GROUP, T.TAXID, T.TAX_MULTI, T.TAX_DIVIDE, T.TAX_DESC FROM article AS P, tax AS T WHERE P.POS_ACTIVE='1' AND P.POS_TAX=T.TAXID AND P.POSITIONID=$PosID");
}
else
{
	$query1 = $db->Execute("SELECT P.POSITIONID, P.POS_NAME, T.POSITIONID, T.POS_QUANTITY, T.POS_DESC, T.POS_PRICE, T.TAX, T.TAX_MULTI, T.TAX_DIVIDE, T.TAX_DESC, T.POS_GROUP FROM article AS P, tmp_offer AS T WHERE T.POSITIONID=P.POSITIONID AND TMP_OFFERID=$tmpPosID");
}
