...
{
	$query = $db->GetRow("SELECT METHODOFPAYID, DESCRIPTION FROM methodofpay WHERE METHODOFPAYID=$MethodOfPayment");
	$METHOD_OF_PAY = $query['DESCRIPTION'];

	$query1 = $db->GetRow("SELECT MESSAGEID, DESCRIPTION FROM message WHERE MESSAGEID=$messageID");
	...
	$query2 = $db->Execute("SELECT PREFIX, TITLE, FIRSTNAME, LASTNAME, ADDRESS, COMPANY, POSTALCODE, PRINT_NAME,
		CITY, COUNTRY, MYID FROM addressbook WHERE MYID=$myID");
...
else
{
	$query = $db->Execute("SELECT A.PREFIX, A.TITLE, A.FIRSTNAME, A.LASTNAME, A.ADDRESS, A.COMPANY, A.POSTALCODE, A.PRINT_NAME,
		...
		FROM addressbook AS A, offer AS O WHERE A.MYID=O.MYID AND O.OFFERID=$offerID");
...
else
{
	if($STATUS != '3')
	{
		$query3 = "UPDATE offer SET STATUS='2', MODIFIEDBY='$_SESSION[Username]', MODIFIED='$CurrentDateTime' WHERE OFFERID=$offerID";
...
if(isset($tmpPos) && $tmpPos == '1')
{
	$posquery = $db->GetAll("SELECT P.POSITIONID, P.POS_NAME, T.USERNAME, T.POSITIONID, T.POS_DESC, T.POS_QUANTITY, T.POS_PRICE, T.TAX, T.TAX_DIVIDE, T.TAX_MULTI, T.TAX_DESC, T.POS_GROUP, T.TMP_OFFERID FROM article AS P, tmp_offer AS T WHERE P.POSITIONID=T.POSITIONID AND T.USERNAME='$_SESSION[Username]' ORDER BY T.POS_GROUP ASC, T.POS_DESC ASC");
}
else
{
	$posquery = $db->GetAll("SELECT P.POSITIONID, P.POS_NAME, V.POSITIONID, V.POS_DESC,
	V.POS_QUANTITY, V.POS_PRICE, V.OFFERID, V.TAX, V.TAX_DIVIDE, V.TAX_MULTI, V.TAX_DESC, V.POS_GROUP FROM article AS P, offerpos AS V WHERE P.POSITIONID=V.POSITIONID AND V.OFFERID=$offerID ORDER BY V.POS_GROUP ASC, V.POS_DESC ASC");
}
