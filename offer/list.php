...
{
	$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, O.CREATEDBY, O.OFFERID, O.MYID, DATE_FORMAT(O.OFFER_DATE,'%d.%m.%Y') AS OFFER_DATE, O.INVOICEID, O.TOTAL_AMOUNT, O.STATUS, O.CANCELED, O.METHODOFPAYID, O.NOTE, O.MESSAGEID FROM offer AS O, addressbook AS A WHERE O.CANCELED=1 AND A.MYID=O.MYID ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
}
else if(isset($Canceled) && $Canceled == 3)
{
	$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, O.CREATEDBY, O.OFFERID, O.MYID, DATE_FORMAT(O.OFFER_DATE,'%d.%m.%Y') AS OFFER_DATE, O.INVOICEID, O.TOTAL_AMOUNT, O.STATUS, O.CANCELED, O.METHODOFPAYID, O.NOTE, O.MESSAGEID FROM offer AS O, addressbook AS A WHERE A.MYID=O.MYID ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
}
else
{
	$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, O.CREATEDBY, O.OFFERID, O.MYID, DATE_FORMAT(O.OFFER_DATE,'%d.%m.%Y') AS OFFER_DATE, O.INVOICEID, O.TOTAL_AMOUNT, O.STATUS, O.CANCELED, O.METHODOFPAYID, O.NOTE, O.MESSAGEID FROM offer AS O, addressbook AS A WHERE O.CANCELED=2 AND A.MYID=O.MYID ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
}
...
if (!$query)
	...
	if(isset($Canceled) && $Canceled == 1)
	{
		$query1 = $db->Execute("SELECT OFFERID, TOTAL_AMOUNT FROM offer WHERE CANCELED=1");
	}
	else if(isset($Canceled) && $Canceled == 3)
	{
		$query1 = $db->Execute("SELECT OFFERID, TOTAL_AMOUNT FROM offer");
	}
	else
	{
		$query1 = $db->Execute("SELECT OFFERID, TOTAL_AMOUNT FROM offer WHERE CANCELED=2");
	}
...
//
$query5 = $db->Execute("SELECT SUM(TOTAL_AMOUNT) AS TOTAL_AMOUNT FROM offer WHERE CANCELED=2 AND STATUS=1");
...
//
$query6 = $db->Execute("SELECT SUM(TOTAL_AMOUNT) AS TOTAL_AMOUNT FROM offer WHERE CANCELED=2 AND STATUS=2");
...
//
$query7 = $db->Execute("SELECT SUM(TOTAL_AMOUNT) AS TOTAL_AMOUNT FROM offer WHERE CANCELED=2 AND STATUS=3");
