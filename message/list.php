...
$smarty->assign("Title","$a[message] - $a[list]");
...
//
$query = $db->Execute("SELECT MESSAGEID, DESCRIPTION FROM message ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT MESSAGEID FROM message");
...
$smarty->display('message/list.tpl');
