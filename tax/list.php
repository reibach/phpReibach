...
if(empty($Order) || $Order !== 'TAXID' && $Order !== 'TAX_DESC')
{
	$Order = "TAX_DESC";
	$Sort = "";
}

$smarty->assign("Title","$a[tax] - $a[list]");
...
// Get Tax Information
//
$query = $db->Execute("SELECT TAXID, TAX_DESC FROM tax ORDER BY $Order $Sort LIMIT $intCursor, $EntrysPerPage");
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT TAXID FROM tax");
