...
$query = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, I.CREATEDBY, I.INVOICEID, I.MYID, DATE_FORMAT(I.INVOICE_DATE,'%d.%m.%Y') AS INVOICE_DATE, SUM(I.TOTAL_AMOUNT) AS TOTAL_AMOUNT, I.PAID, SUM(I.SUM_PAID) AS SUM_PAID, SUM(I.TOTAL_AMOUNT-I.SUM_PAID) AS OPEN_ACCOUNT, I.CANCELED FROM invoice AS I, addressbook AS A WHERE A.MYID=I.MYID AND I.CANCELED=2
...
if (!$query)
	...
	// Count MaxRows
	//
	$query1 = $db->Execute("SELECT A.MYID, A.FIRSTNAME, A.LASTNAME, A.COMPANY, I.INVOICEID, I.MYID, SUM(I.TOTAL_AMOUNT) AS TOTAL_AMOUNT, SUM(I.SUM_PAID) AS SUM_PAID FROM addressbook AS A, invoice AS I WHERE A.MYID=I.MYID AND I.CANCELED=2
